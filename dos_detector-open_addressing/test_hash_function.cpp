    ...
    try
    {
        ...
        while(std::getline(input_file, buffer))
        {
            ...
            else if (command=="LP_HASH")
            {
                ...
                if (!input)
                    throw std::runtime_error("Wrong command format.");
                LPHash hash (UHash::create(m, p, a, b));
            ...
            else if (command=="QP_HASH")
            {
                ...
                if (!input)
                    throw std::runtime_error("Wrong command format.");
                QPHash hash (UHash::create(m, p, a, b));
            ...
            else if (command=="RP_HASH")
            {
                ...
                if (!input)
                    throw std::runtime_error("Wrong command format.");
                RPHash hash (UHash::create(m, p, a, b), c);
            ...
            else if (command=="D_HASH")
            {
                ...
                if (!input)
                    throw std::runtime_error("Wrong command format.");
                DHash hash (UHash::create(m, p, a[0], b[0]),
                            UHash::create(m, p, a[1], b[1]));
                std::cout << "Hashing key " << k << " (double hashing probing):";
                for (size_t i=0;i<m; ++i)
                    std::cout << ' ' << hash(k, i);
                std::cout << std::endl;
            }
